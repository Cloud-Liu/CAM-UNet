...
class CAM_UNet(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        self.encoder1=CAM_Bottleneck(3,64,64)
        self.encoder2=CAM_Bottleneck(64,128,128)
        self.encoder3=CAM_Bottleneck(128,256,256)
        self.encoder4=CAM_Bottleneck(256,512,512)
        self.encoder5=CAM_Bottleneck(512,1024,1024)
        ...
        self.decoder1=CAM_Bottleneck(1024,512,512)
        self.decoder2=CAM_Bottleneck(512,256,256)
        self.decoder3=CAM_Bottleneck(256,128,128)
        self.decoder4=CAM_Bottleneck(128,64,64)
        self.decoder5=CAM_Bottleneck(64,32,32)
    ...
    def forward(self, x):
        ...
        out = F.interpolate(self.dbn1(self.decoder1(out)),scale_factor=(2,2),mode ='bilinear')
        ra = (((- 1) * torch.sigmoid(out)) + 1)
        ra = ra.expand((- 1), 512, (- 1), (- 1)).multiply(t4)#t4:torch.Size([2, 512, 32, 32])
        # ra=self.bn5(ra)
        t4=self.relu(ra)
        # t4=ra+t4
        ...
        out = F.interpolate(self.dbn2(self.decoder2(out)),scale_factor=(2,2),mode ='bilinear')
        ra = (((- 1) * torch.sigmoid(out)) + 1)
        ra = ra.expand((- 1), 256, (- 1), (- 1)).multiply(t3)#t4:torch.Size([2, 512, 32, 32])
        # ra=self.bn5(ra)
        t3=self.relu(ra)
        ...
        out = F.interpolate(self.dbn3(self.decoder3(out)),scale_factor=(2,2),mode ='bilinear')
        ra = (((- 1) * torch.sigmoid(out)) + 1)
        ra = ra.expand((- 1), 128, (- 1), (- 1)).multiply(t2)#t4:torch.Size([2, 512, 32, 32])
        # ra=self.bn5(ra)
        t2=self.relu(ra)
        out = torch.add(out,t2)
        out3= F.interpolate(self.final(self.final3(out)),scale_factor=(4,4),mode ='bilinear')
        out = F.interpolate(self.dbn4(self.decoder4(out)),scale_factor=(2,2),mode ='bilinear')
        ra = (((- 1) * torch.sigmoid(out)) + 1)
        ra = ra.expand((- 1), 64, (- 1), (- 1)).multiply(t1)#t4:torch.Size([2, 512, 32, 32])
        # ra=self.bn5(ra)
        t1=self.relu(ra)
        out = torch.add(out,t1)
        # out4= F.interpolate(self.final(self.final4(out)),scale_factor=(2,2),mode ='bilinear')
